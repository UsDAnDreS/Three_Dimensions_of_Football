#####
## ADDING THE EXPECTED POINTS
##    -  "ep_before", "ep_after", "EPA"
#####


## (APRIL 6th)  LINE 6890: COME BACK TO DO THE CHECKS of >100 RUSH/PASS YARDS, PLUS THE 1st/3rd/4th DOWN STUFF AT THE VERY END
##                         ALSO CHECK THE PENALTY YARDAGE SUMMARIES (AFTER THE NAs got introduced for RIDICULOUS SINGLE PLAY PENALTY VALUES)

##
## LINES 6630+: LOOK AT HOW POINTS SCORED ARE CALCULATED for EACH DRIVE
##    ALSO CHECK THE NEGATIVE PLAYS, STUFFED RUNS, RED ZONE VISITS/CONVERSIONS, PASS BREAKUPS

## MARCH 19th: NOTE - I HAVE DECIDED to NOT drop the pure "END OF HALF/GAME" ONE-PLAY 'DRIVES', just UNTIL THE VERY END when it's FULLY NECESSARY
##             OTHERWISE it might "MESS WITH THE 'ECOSYSTEM' OF THIS DATASET

### LINE 5894, ABOUT SAVING SOME BAD "POS TEAM SWITCHING" HALVES !!!
##
## NOTE FOR FUTURE: DON'T TRUST THE DRIVE VARIABLES besides "drive_number", "drive_result_detailed", "drive_id", "game_id_drive" and maaaaybe "drive_play_number"
##    THE REST ARE QUESTIONABLE (e.g. TIME ELAPSED, STARD/END YARDS TO GOAL, ETC ETC), ESPECIALLY WITH ALL OF OUR MANIPULATIONS
##
##  Those "DRIVE SUMMARY" VARIABLES MIIIIIGHT BE USEFUL FOR "LAZY DRIVE-BY-DRIVE" ANALYSIS, if I END UP GIVING UP ON CLEANING THE DATA



########
## CHECK IF THERE ARE ""Kickoff Team Fumble Recovery TOUCHDOWN" plays in OTHER YEARS (it's NOT in 2014)
## PLUS CHECK IF "KICKOFF (SAFETY)" EVER ENDS UP SHOWING UP (it seemed to be there in 2014 at first, but got wiped out...)
########
...
## WILL PROBABLY NEED TO CHANGE IT WHEN SUMMARIZING THE DRIVES...
######
...
#######
##  NOTE: be mindful of the ".groups" argument in "summarise()"
##     I set it to "keep" in cases of multi-row-per-group scenario in order to avoid the generated printouts, 
##     and it SHOULD be good to go, but one never fully knows.
#######

## !!! KEEP IN MIND - HANDFUL OF THE GAMES END UP NOT HAVING AN "OFFICIAL" 'END OF 2nd quarter', 'End of 4th quarter' PLAY.. !!!

##
## !! INCOMPLETIONS STILL HAVE "YDS_RECEIVING"/"RECEPTION_YDS" AT NA !!!
## !! SO WE CAN'T IDENTIFY PASSING PLAYS SIMPLY BY "is.na(yds_receiving)" !!!
##


#######
## KEEP IN MIND that DROPPING OBSERVATIONS such as, e.g. "Timeout", "End of 1st/3rd quarter", 
## IS NOT GONNA BE REFLECTED IN LEAD/LAG FIELDS, and IT'S PROBABLY NOT A GOOD IDEA TO MAKE IT REFLECTED EITHER...
## (some info could be useful due to the way the data was entered, e.g. "NA" for yards_to_goal, etc)
## JUST USE "lead()/lag()" FUNCTIONS
########


##  NOTE: "NA" ARE POSSIBLE for the DRIVE_RESULT_DETAILED whenever there's NO DRIVE AFTER THE "FUMBLE RECOVERY"
##  (when offense fumbles, it gets taken by the opponent, but then instantly returns to the offense)
##  DECIDED TO KEEP THOSE "NA" INSTEAD OF RISKING

#######
## DRIVES DISAPPEAR:
## (bad.switch.halves.vol.3)
##  #  400547905-1 - definitely something amiss here.. drive number skips 11 to 13
##     400547761-2, 400548086-2 - straight up missing a drive.. skipping a drive number too (13 to 15)
##
##  dim(pbp[pbp$game_id_drive == "400547905-12",])
##  ALTHOUGH AT THE START, THERE WERE DRIVES IN-BETWEEN THOSE !! 
##
## ALSO, PONDER WHETHER TO EXCLUDE THE GAMES that SKIP DRIVE NUMBERING 
## (e.g. go 11 to 13, although it's on the instantaneous defensive score plays, and doesn't seem to be incorrect..)
#######

####
#      WHEN DEALING WITH WEIRD ONE-PLAY POSSESSIONS, check for "PENALTY" BEING AS THE LAST PLAY OF THE PRECEDING POSSESSION
#      (PLUS ALSO DO THAT WHEN CHECKING "DRIVE_RESULT_DETAILED = LAST PLAY TYPE")
##     WHEN CHANGING DRIVE NUMBERS by +- 0.5, REMEMBER ABOUT "PENALTIES" POTENTIALLY GETTING LEFT HANGING as a SINGLE POSSESSION
#####

## NOTE: "yards_to_goal" is AT THE BEGINNING OF THE PLAY (as opp to "yards_to_goal_end")


##########
##########
##POTENTIAL ISSUE FOR FUTURE CONSIDERATION: 
##      For PUNTS: change "yards_to_goal" to "100 - yards_to_goal", as we SWITCHED POSSESSION for those
##########
##########
...
#######
# "filter()" in TIDYVERSE drops NA rows... e.g. "NA != 80" is evaluated to NA, not TRUE.. and it DROPS IT..
########

##########
## 5. Checking to YARDS_TO_GOAL_END of current observation vs YARDS_TO_GOAL of the NEXT OBSERVATIONS
## !!! That kills a HUGE AMOUNT OF HALVES (500 out of 1450.. a THIRD..) !!!
##########

### SOME GAMES (only a couple) MISS THAT LAST "END OF GAME" ROW...
### !!! SO, WHEN LOOKING FOR THE END OF EACH RESPECTIVE GAME, DON'T JUST LOOK FOR "END OF HALF/GAME", ALSO JUST CHECK THE GAME_ID !!!


######
## The TEAM-CHANGES within SAME DRIVE for "Fumble Return TD" => many of those are YOUR OWN RECOVERED FUMBLE FOR A TD...
## Need to figure out how to FLIP THE TEAM NOTATION (see 1267), and also KEEP TRACK OF YARDS_GAINED (see 835)
########
...
## Make sure to NOT SEPARATE THE "DRIVE" & "GAME_ID" with a ".", use OTHER SIGNS !!!

#########
## 1. !!! WATCH OUT for "END OF QUARTER/HALVES" BEING TREATED AS WHOLE DRIVES, getting THEIR OWN DRIVE NUMBERS !!!!
##
## 2. RETAIN HALVES, rather than GAMES.. don't want to THROW OUT AN ENTIRE GAME based on some BAD OBSERVATIONS in ONE HALF.
##
## 3. !!! CHECK FOR DRIVES where KICKOFF APPEARS ANYWHERE BUT THE FIRST PLAY OF THE DRIVE !!!!
##       (there seems to be random cases where it's put AT THE END or SOMEWHERE IN THE MIDDLE of a drive...)
## 
## 4. Check to D of current observation vs YARDS_TO_GOAL of the NEXT OBSERVATIONS
#########
...
# load("pwp_week1.Robj")
# load("pwp_on_campus_full_data.Robj")
...
for (year in 2014:2020){
  ...
  ####
  ## EXPECTED POINTS
  ##    - EPA is incorrectly calculated on plays where there's switch in possession, e.g. on Punts...
  ##    - "ep_after" also doesn't always work as needed... sometimes isn't equal to "ep_before" on the next one, even on NON-SCORING PLAYS
  ##    - JUST TAKE "ep_before", and then we'll FIGURE OUT THE TENDENCIES BETWEEN VARIOUS OUTCOMES
  ####

# our.ind <- which(#pbp$ep_after != lead(pbp$ep_before) & 
#                    pbp$play_type == "Blocked Punt")
# 
...
#                 pos_team, def_pos_team))
  ...
  if (length(kickoff.penalty.plays) > 0){
    ...
    ## flip the sign for "ep_before", "ep_after" 
    #pbp[kickoff.penalty.plays, c("ep_before", "ep_after")] <- -pbp[kickoff.penalty.plays, c("ep_before", "ep_after")]
  }
  ...
  # View(pbp %>% select(game_id, drive_number, drive_result_detailed))
  ...
  pbp_by_drive[our.ind, ]$score_pts <- ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), fixed("two")) & !str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), "failed"),
                                              8,
                                              ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), fixed("kick)")),
                                                     7,
  ...
  print(table(score_pts = pbp_by_drive$score_pts_by_text !=0 , drive_res =pbp_by_drive$drive_result_detailed %in% scoring_drive_results))
  ...
  score.ind <- which(pbp_by_drive$score_pts_by_text != 0)
  pbp_by_drive$ep_after[score.ind] <- pbp_by_drive$score_pts_by_text[score.ind]
  pbp_by_drive$EPA.diff[score.ind] <- pbp_by_drive$ep_after[score.ind] - pbp_by_drive$ep_before[score.ind]
  ...
  # 
  # i <- 23
  # print(sort(unique(pbp_by_drive$drive_result_detailed))[i])
  # our.ind <- which(pbp_by_drive$drive_result_detailed == sort(unique(pbp_by_drive$drive_result_detailed))[i])
  # print(summary(pbp_by_drive$ep_after[our.ind]))
  # print(summary(pbp_by_drive$ep_before[our.ind+1]))
  # print(summary(pbp_by_drive$EPA.diff[our.ind]))
  # 
  # #  our.ind[which.max(pbp_by_drive$ep_after[our.ind])]
  # 
  # View(pbp_by_drive[
  #   #c(our.ind[which.min(pbp_by_drive$ep_after[our.ind])], our.ind[which.min(pbp_by_drive$ep_after[our.ind])]+1)
  #   c(our.ind[which.min(pbp_by_drive$ep_before[our.ind+1])], our.ind[which.min(pbp_by_drive$ep_before[our.ind+1])]+1)
  #   , ] %>%
...
table(pbp_by_drive$score_pts_by_text[pbp_by_drive$drive_result_detailed == sort(unique(pbp_by_drive$drive_result_detailed))[i]])
