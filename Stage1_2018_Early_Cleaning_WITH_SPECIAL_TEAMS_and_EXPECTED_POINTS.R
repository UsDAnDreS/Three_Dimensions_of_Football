...
for (year in 2017){
  ...
  ## Switching ep's before & after accordingly,
  ##  as the "ep_after" seems to pertain to the kicking team, not the returning one (although ep_before applies to returning)
  # pbp[ind.play.type-1, "ep_after"] <- -pbp[ind.play.type-1, "ep_after"]
  pbp[onside.lost.play.id, "ep_after"] <- -pbp[onside.lost.play.id, "ep_after"]
  pbp[onside.lost.play.id, "EPA"] <- pbp[onside.lost.play.id, "ep_after"] - pbp[onside.lost.play.id, "ep_before"]
  ...
  ## Switching ep's before & after accordingly
  # pbp[ind.play.type-1, "ep_after"] <- -pbp[ind.play.type-1, "ep_after"]
  pbp[ind.play.type, "ep_before"] <- -pbp[ind.play.type, "ep_before"]
  pbp[ind.play.type, "EPA"] <- pbp[ind.play.type, "ep_after"] - pbp[ind.play.type, "ep_before"]
  ...
  ## Switching ep's before & after accordingly
  ##  HERE it needs to also switch for the AFTER yardage, as both are being considered as "punting team's possession" - hence both need to be flipped
  pbp[ind.play.type, "ep_before"] <- -pbp[ind.play.type, "ep_before"]
  pbp[ind.play.type, "ep_after"] <- -pbp[ind.play.type, "ep_after"]
  pbp[ind.play.type, "EPA"] <- pbp[ind.play.type, "ep_after"] - pbp[ind.play.type, "ep_before"]
  ...
  #####
  ## EXP PTS
  ##  CHECKING OBSERVATIONS WHERE THE "ep_after" doesn't match with next observation's "ep_before"
  #####
  #####
  ...
  cat("\n")
  cat("\n")
  print("# of bad EXPECTED POINTS observations:")
  
  
  ## Those where "ep_after" and next play's "ep_before" are just with FLIPPED SIGNS
  
  ## 1. Where the POS_TEAM didn't change:
  ##      Blocked punts, Blocked FG that instantly got back to the kicking team
  ##      The "ep_after" probably should've been flipped
  our.ind <- which(pbp$ep_after == -lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     pbp$pos_team == lead(pbp$pos_team))
  our.ind
  # print(length(our.ind))
  
  
  # Just flip the sign for "ep_after", recalculate EPA
  pbp$ep_after[our.ind] <- -pbp$ep_after[our.ind]
  pbp[our.ind, "EPA"] <- pbp[our.ind, "ep_after"] - pbp[our.ind, "ep_before"]
  
  table(pbp$play_type[our.ind])
  
  
  
  # 2. Where the POS_TEAM changed:
  #     That's what it's supposed to do. Those are fine.
  
  our.ind <- which(pbp$ep_after == -lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     pbp$pos_team != lead(pbp$pos_team))
  our.ind
  
  ## Offensive TDs where "ep_after" is not 7:
  ##    - Switch to 7.
  ##    - Recalculate EPA
  
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     (pbp$play_type %in% offensive.TD.score.drive.res) & !pbp$ep_after %in% c(7))
  our.ind
  
  pbp$ep_after[our.ind] <- 7
  pbp$EPA[our.ind] <- pbp$ep_after[our.ind] - pbp$ep_before[our.ind]
  
  print("Offensive TD's where 'ep_after' is not 7 (fixable):")
  print(length(our.ind))
  
  ## Defensive TDs where "ep_after" is not -7:
  ##    - Switch to -7.
  ##    - Recalculate EPA
  
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     (pbp$play_type %in% defensive.TD.score.drive.res[defensive.TD.score.drive.res != "Safety"]) & !pbp$ep_after %in% c(-7))
  our.ind
  
  pbp$ep_after[our.ind] <- -7
  pbp$EPA[our.ind] <- pbp$ep_after[our.ind] - pbp$ep_before[our.ind]
  
  print("Defensive TD's where 'ep_after' is not -7 (fixable):")
  print(length(our.ind))
  
  
  View(pbp[sort(c(our.ind, our.ind+1
                  #, our.ind-1
                  #, our.ind+2, our.ind+3
  )), 
  c("game_id_drive", "clock.minutes", "clock.seconds", "period", "down", "distance", "yards_to_goal", "play_text", "play_type", "ep_before", "ep_after", "EPA", "pos_team", "def_pos_team")])
  
  
  
  
  ## Touchdowns, where "ep_after" is not 7 or -7
  # our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
  #                    str_detect(pbp$play_type, "Touchdown") & !pbp$ep_after %in% c(7,-7))
  # our.ind
  # 
  # # Just assign the "score_pts" to ep_after, and then adjust EPA
  # pbp$ep_after[our.ind] <- pbp$score_pts[our.ind]
  # pbp$EPA[our.ind] <- pbp$ep_after[our.ind] - pbp$ep_before[our.ind]
  ...
  print("TD's where 'ep_after' is not 7 or -7:")
  print(length(our.ind))
  ...
  ## Made FGs, where "ep_after" is not 3
  print("FG's where 'ep_after' is not 3:")
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     str_detect(pbp$play_type, "Field Goal Good") & !pbp$ep_after %in% c(3))
  our.ind
  print(length(our.ind))
  
  print("Safety's where 'ep_after' is not -2:")
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     str_detect(pbp$play_type, "Safety") & !pbp$ep_after %in% c(-2))
  our.ind
  print(length(our.ind))
  
  ####
  ## The "ep_before = 0" cases
  ####
  
  ## EPA: for some reason does "EPA" is calculated based on "0 - ep_before", even though "ep_after" shows non-0..
  ##  Recalculate EPA as a difference for that row
  ##  If the last play ended in a TD: assign the EPA to "ep_after". Otherwise - NA
  
  our.ind <- which(#pbp$ep_after != lead(pbp$ep_before) & 
    (pbp$game_id_half == lead(pbp$game_id_half)) 
    & (lead(pbp$ep_before) %in% c(0)) & (lead(pbp$EPA) %in% c(-7,7))
  )
  our.ind
  
  pbp$EPA[our.ind] <- pbp$ep_after[our.ind] - pbp$ep_before[our.ind]
  pbp$ep_after[our.ind+1] <- pbp$EPA[our.ind+1]
  ...
  pbp$EPA[our.ind] <- pbp$ep_after[our.ind] - pbp$ep_before[our.ind]
  pbp$ep_after[our.ind+1] <- NA
  
  
  
  
  ####
  ## The EP_BEFORE & EP_AFTER NOT SUMMING UP TO EPA
  ##
  ##    1. The ones where EPA is just set equal to "-ep_before":
  ##          For pre-"End of Half", where "ep_after" should be 0 => just switch "ep_after" to 0.
  ##          For others - NA, those are bad.
  ##
  ##    2. Other plays: just bad ones.. so far none of them, but if those exist, might need to set to NA
  ####
  
  our.ind <- which(#pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) 
    abs(pbp$EPA - (pbp$ep_after - pbp$ep_before)) > 10^{-8} 
    & pbp$EPA == -pbp$ep_before
    & str_detect(lead(pbp$play_type), "End of")
    
    # & str_detect(pbp$play_type, "Penalty")
  )
  our.ind
  table(pbp$play_type[our.ind])
  
  pbp$ep_after[our.ind] <- 0
  ...
  our.ind
  print("Non-'End of Half' EPA=-ep_before cases (bad, set to NA)")
  print(length(our.ind))
  table(pbp$play_type[our.ind])
  
  pbp$ep_after[our.ind] <- NA
  pbp$EPA[our.ind] <- NA
  
  
  
  ## That aren't a "EPA = -ep_before"
  ...
  print("Number of bad EPAs that didn't result from setting EPA to '-ep_before':")
  print(length(our.ind))
  cat("\n")
  cat("\n")
  ...
  ## Penalties 
  ##    Many of these don't make sense... There's no change in clock, yet the "ep_after != next play's ep_before"..
  ##    but maybe some are due to STATIONARY CLOCK...
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     !(pbp$ep_after == -lead(pbp$ep_before)) &
                     str_detect(pbp$play_type, "Penalty"))
  our.ind
  ...
  print("Amount of semi-shaky post-penalty EP/EPA:")
  print(length(our.ind))
  
  
  #which( abs(pbp[our.ind, c("EPA")] - (pbp[our.ind, "ep_after"] - pbp[our.ind, "ep_before"])) > 10^{-8})
  
  
  
  ## Non-penalties:
  ##    1. Scoring plays (Touchdowns, made FGs, Safeties, makes sense). These are fine, shouldn't carry over
  ##    2. Non-scoring plays
  
  our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
                     !(pbp$ep_after == -lead(pbp$ep_before)) &
                     #abs(pbp$EPA - (pbp$ep_after - pbp$ep_before)) > 10^{-8} & 
                     str_detect(pbp$play_type, "Touchdown") | str_detect(pbp$play_type, "Field Goal Good") | str_detect(pbp$play_type, "Safety")
  )
  our.ind
  
  
  ## Checking all cases where EP_AFTER and next play's EP_BEFORE are more than 0.2 apart
  ## (with POS team NOT SWITCHING)
  
  # our.ind <- which(pbp$ep_after != lead(pbp$ep_before) & (pbp$game_id_half == lead(pbp$game_id_half)) &
  #                    !(pbp$ep_after == -lead(pbp$ep_before)) & !(lead(pbp$ep_before) == 0) &
  #                    (abs(pbp$ep_after - lead(pbp$ep_before)) > 0.2) & (pbp$pos_team == lead(pbp$pos_team))  &
  #                    #abs(pbp$EPA - (pbp$ep_after - pbp$ep_before)) > 10^{-8} & 
  #                    !str_detect(pbp$play_type, "Penalty") &
  #                    !(str_detect(pbp$play_type, "Touchdown") | str_detect(pbp$play_type, "Field Goal Good") | str_detect(pbp$play_type, "Safety"))
  # )
  ...
  # our.ind
  # 
  # print("Amount of semi-shaky NON post-penalty/scoring yardages:")
  # print(length(our.ind))
  # 
  # table(pbp$play_type[our.ind])
  ...
  ## Doing the POST-ST PLAY PENALTIES??? CAN'T... it's too tough to distinguish those.
  ...
  pbp_by_drive_full <- pbp_by_drive %>%
    ...
    summarize(n.pass_attempts = sum(pass_attempt, na.rm=T),
              ...
              ## All the expected point metrics
              ep_before = head(ep_before,1),
              ep_after = tail(ep_after,1),
              EPA.sum = sum(EPA),
              EPA.diff = ep_after - ep_before,
  ...
  if (year == 2017){
    ...
    pbp_by_drive_full[bad.ind, "score_pts"] <- 0
  ...
  # our.ind <- which((pbp_by_drive_full$score_pts != 0) & !(pbp_by_drive_full$drive_result_detailed %in% scoring_drive_results))
                   # & !(str_detect( lead(pbp_by_drive_full$drive_result_detailed), "Touchdown") | str_detect( lead(pbp_by_drive_full$drive_result_detailed), "Field Goal")  ))

  View(pbp_by_drive_full[sort(c(our.ind, our.ind-1, our.ind + 1)),] %>%
         select(score_pts, drive_result_detailed, first.play.text.no.pen, first.play.text.with.pen, last.play.text.no.pen, last.play.text.with.pen)
         )
  ...
  pbp_by_drive_full$rz_conversion <- pbp_by_drive_full$rz_visit & (pbp_by_drive_full$score_pts_by_text > 0)
  
  
  print("Points scored, with 'score_pts_by_text', PAY CLOSE ATTENTION TO OFF-DIAGONALS")
  print(table(score_pts = pbp_by_drive_full$score_pts_by_text !=0 , drive_res =pbp_by_drive_full$drive_result_detailed %in% scoring_drive_results))
  ...
  print("Rush yds:")
  print(summary(pbp_by_drive_full$tot.yds_rushed))
  print("Pass yds:")
  print(summary(pbp_by_drive_full$tot.yds_receiving))
  
  print("TOTAL drives with >100 rush or pass yds ")
  our.ind <- which(pbp_by_drive_full$tot.yds_rushed > 100 | pbp_by_drive_full$tot.yds_receiving > 100)
  print(length(our.ind))
  print("OFFENSIVE PENALTY yardages in drives with >100 rush or pass yds")
  if (length(our.ind) > 0) print(pbp_by_drive_full[our.ind,]$tot.yds_penalty)
  print("NEGATIVE PLAYS (with penalties) in drives with >100 rush or pass yds")
  if (length(our.ind) > 0) print(pbp_by_drive_full[our.ind,]$n.negative.plays.with.penalties)
  
  print("# PROBLEMATIC drives with >100 rush or pass yds and 0 OFFENSIVE PENALTIES AND 0 NEGATIVE PLAYS IN GENERAL")
  our.ind <- which((pbp_by_drive_full$tot.yds_rushed > 100 | pbp_by_drive_full$tot.yds_receiving > 100) & (pbp_by_drive_full$penalties.offense == 0 & pbp_by_drive_full$n.negative.plays.with.penalties == 0 ))
  print(length(our.ind))
  
  
  print("Rushing attempts:")
  print(summary(pbp_by_drive_full$n.rush))
  print("Stuffed runs:")
  print(table(pbp_by_drive_full$n.stuffed.runs))
  
  print("Passing attempts:")
  print(summary(pbp_by_drive_full$n.pass_attempts))
  print("Pass completions:")
  print(summary(pbp_by_drive_full$n.completions))
  print("Pass incompletions:")
  print(summary(pbp_by_drive_full$n.incompletions))
  print("Pass breakups")
  print(table(pbp_by_drive_full$n.pass.breakups))
  
  
  print("Summary of negative plays, NO PENALTIES:")
  print(table(pbp_by_drive_full$n.negative.plays.no.penalties))
  print("Summary of negative plays, WITH PENALTIES:")
  print(table(pbp_by_drive_full$n.negative.plays.with.penalties))
  
  print("Penalties count, offense:")
  print(summary(pbp_by_drive_full$penalties.offense))
  print("Penalties count, defense:")
  print(summary(pbp_by_drive_full$penalties.defense))
  
  
  
  print("Red zone visits & conversions:")
  print(table(pbp_by_drive_full$rz_visit))
  print(table(pbp_by_drive_full$rz_conversion))
  
  print("Fumble totals:")
  print(table(pbp_by_drive_full$n.fumbles))
  
  print("Sack totals:")
  print(table(pbp_by_drive_full$n.sacks))
  
  print("Sack yards:")
  print(summary(pbp_by_drive_full$tot.yds_sacked))
  
  print("Penalty yards:")
  print(summary(pbp_by_drive_full$tot.yds_penalty))
  
  print("First downs gained:")
  print(summary(pbp_by_drive_full$first.downs.gained))
  
  print("Third downs attempted:")
  print(summary(pbp_by_drive_full$third.downs.total))
  
  print("Third downs converted:")
  print(summary(pbp_by_drive_full$third.downs.converted))
  
  print("Fourth downs attempted:")
  print(summary(pbp_by_drive_full$fourth.downs.total))
  
  print("Fourth downs converted:")
  print(summary(pbp_by_drive_full$fourth.downs.converted))
  ...
  print(sum(pbp_by_drive_full$n.plays.no.penalties > pbp_by_drive_full$n.plays.with.penalties))
  ...
  ## FG RETURNS:
  # View(pbp_by_drive_full %>% filter(!is.na(yds_fg_return)) %>%
  ...
  summary(pbp_by_drive_full$off.yards_gained)
  
  ## ALL YARDS (including PUNTS, KICKOFFS, FG etc)
  # THAT ONE IS SHAKY...
  summary(pbp_by_drive_full$tot.yards_gained)
  ...
  summary(pbp_by_drive_full$tot.yds_fumble_return[pbp_by_drive_full$n.fumbles >= 1])
  summary(pbp_by_drive_full$tot.yds_fumble_return)
  
  summary(pbp_by_drive_full$tot.yards_gained[pbp_by_drive_full$n.fumbles >= 1])
  ...
  our.ind <- which(pbp_by_drive_full$game_id_half == lead(pbp_by_drive_full$game_id_half) & is.na(pbp_by_drive_full$stopped_position) &
                     lead(pbp_by_drive_full$n.plays.with.penalties) != 0)
  our.ind
  ...
  sum(is.na(pbp_by_drive_full$stopped_position))
  length(unique(pbp_by_drive_full$game_id_half))
  
  
  # View(#pbp_by_drive_full[sort(c(our.ind)), ] %>%
  #   pbp_by_drive_full[sort(c(our.ind, our.ind+1)), ] %>%
  #     #pbp_by_drive_full %>% filter(game_id_drive %in% c("400547640-12")) %>%
  ...
  stat <- sort(unique(pbp_by_drive_full$drive_result_detailed))[j]
  sum(pbp_by_drive_full$drive_result_detailed == stat)
  
  stat
  stop.pos.vec <- pbp_by_drive_full %>% 
  ...
  summary(pbp_by_drive_full %>% filter(drive_result_detailed == "Punt") %>% .[["stopped_position"]]) # %>% summary()
  our.ind <- which(pbp_by_drive_full$drive_result_detailed == "Punt" & pbp_by_drive_full$stopped_position >= 80) 
  ...
  # boxplot(pbp_by_drive_full %>% filter(drive_result_detailed == "Field Goal Missed") %>% .[["stopped_position"]]) # %>% summary()
  print("Number of somewhat problematic FG Missed Stopped Positions (>80):")
  print(nrow(pbp_by_drive_full %>% filter(drive_result_detailed == "Field Goal Missed", stopped_position > 80)))
  
  print("Number of REALLY problematic FG Missed Stopped Positions (<= 30):")
  print(nrow(pbp_by_drive_full %>% filter(drive_result_detailed == "Field Goal Missed", stopped_position <= 30)))
  ...
  print(sum(abs(pbp_by_drive_full$tot.yards_gained) > 100))
  print("# of PROBLEMATIC (over 100) yards gained by OFFENSE ONLY:")
  print(sum(abs(pbp_by_drive_full$off.yards_gained) > 100))
  
  print("# of REALLY PROBLEMATIC OFFENSE+SPECIAL TEAMS YARDAGES (> 110):")
  our.ind <- which(pbp_by_drive_full$off.yards_gained + ifelse(!is.na(pbp_by_drive_full$yds_punt_return),
  ...
  print(sum(abs(pbp_by_drive_full$tot.yds_rushed) > 100))
  print("# of drives w >100 yards gained by PASSING, not really problematic (can happen due to penalties):")
  print(sum(abs(pbp_by_drive_full$tot.yds_receiving) > 100))
  ...
  print(sum(abs(pbp_by_drive_full$tot.yds_penalty) > 100))
  ...
  #   
  # #pbp_by_drive_full[sort(c(our.ind)), ] %>%
  # pbp_by_drive_full[sort(c(our.ind, our.ind-1, our.ind+1)), ] %>%
  # 
  ...
  # View(pbp_by_drive_full %>% filter(abs(yards_gained) > 100))
  ...
  our.ind <- which((pbp_by_drive_full$tot.na.yards_gained > 0 | pbp_by_drive_full$off.na.yards_gained > 0) & 
  ...
  summary(pbp_by_drive_full$drive_duration_seconds)
  summary(pbp_by_drive_full$drive_time_minutes_start)
  summary(pbp_by_drive_full$drive_time_seconds_start)
  ...
  our.ind <- which(is.na(pbp_by_drive_full$drive_duration_seconds) & pbp_by_drive_full$n.plays.with.penalties %in% c(0,1))
  
  print("# of NA-drive time observations being set to 0:")
  print(length(our.ind))
  if (length(our.ind) > 0){
    pbp_by_drive_full$drive_duration_seconds[our.ind] <- 0
  }
  
  bad.ind <- which(is.na(pbp_by_drive_full$drive_duration_seconds) & pbp_by_drive_full$n.plays.with.penalties > 1)
  ...
  print(sum(pbp_by_drive_full$drive_duration_seconds < 0))
  # boxplot(pbp_by_drive_full$drive_duration_seconds)
  
  ## "-15:00" types of cases, which could be parsed out and added a 900:
  pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds <0 &
                 pbp_by_drive_full$drive_time_minutes_end == 15 &
                 pbp_by_drive_full$drive_time_seconds_end == 00, "drive_duration_seconds"] <- 900 + pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds <0 &
                                                                                                              pbp_by_drive_full$drive_time_minutes_end == 15 &
                                                                                                              pbp_by_drive_full$drive_time_seconds_end == 00, "drive_duration_seconds"]
  ...
  # pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds <0, "drive_duration_seconds" ] <- 0
  # # pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds <0, "drive_duration_seconds" ] <- - pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds <0, "drive_duration_seconds"]
  # sort(pbp_by_drive_full$drive_duration_seconds)[1:10]
  ...
  table(pbp_by_drive_full$drive_result_detailed[pbp_by_drive_full$drive_duration_seconds == 0 & !str_detect(pbp_by_drive_full$drive_result_detailed, "Punt")])
  
  ## Checking N OF PLAYS:
  table(pbp_by_drive_full$n.plays.with.penalties[pbp_by_drive_full$drive_duration_seconds == 0])
  table(pbp_by_drive_full$n.plays.no.penalties[pbp_by_drive_full$drive_duration_seconds == 0])
  
  ## Checking the MULTI-PLAY drives with 0 DURATION
  ##   Vastly 1-play, or 2-play ones that are OK for a 0-second duration.
  
  pbp_by_drive_full[pbp_by_drive_full$drive_duration_seconds == 0 & pbp_by_drive_full$n.plays.with.penalties > 1, ]
  
  
  print("Number of PROBLEMATIC 0-DURATION drives with >2 PLAYS (OVERALL):")
  print(sum(pbp_by_drive_full$n.plays.with.penalties[pbp_by_drive_full$drive_duration_seconds == 0] > 2))
  ...
  our.ind <- which(pbp_by_drive_full$drive_duration_seconds == 0 & 
                     pbp_by_drive_full$drive_time_minutes_end == 0 & pbp_by_drive_full$drive_time_seconds_end == 0)
  print(length(our.ind))
  
  pbp_by_drive_full[our.ind, "drive_duration_seconds"] <- NA
  ...
  our.ind <- which(pbp_by_drive_full$drive_duration_seconds == 0 & 
                     !(pbp_by_drive_full$drive_time_minutes_end == 0 & pbp_by_drive_full$drive_time_seconds_end == 0) &
                     pbp_by_drive_full$n.plays.with.penalties > 2 & 
                     !pbp_by_drive_full$stagn.clock & 
                     pbp_by_drive_full$game_id_half == lead(pbp_by_drive_full$game_id_half))
  
  if (length(our.ind) > 0){
    pbp_by_drive_full[our.ind, "drive_duration_seconds"] <- 900*(pbp_by_drive_full[our.ind+1, c("drive_time_period_start")] - pbp_by_drive_full[our.ind, c("drive_time_period_start")]) +
      60*(pbp_by_drive_full[our.ind, c("drive_time_minutes_start") ] - pbp_by_drive_full[our.ind+1, c("drive_time_minutes_start") ]) +
      pbp_by_drive_full[our.ind, c("drive_time_seconds_start") ] - pbp_by_drive_full[our.ind+1, c("drive_time_seconds_start")]
  }
  
  ## If next drive is FROM A DIFFERENT HALF:
  ##   Just do the conversion of time left of the current drive into duration
  our.ind.next.end.of.half <- which(pbp_by_drive_full$drive_duration_seconds == 0 & 
                                      !(pbp_by_drive_full$drive_time_minutes_end == 0 & pbp_by_drive_full$drive_time_seconds_end == 0) &
                                      pbp_by_drive_full$n.plays.with.penalties > 2 & 
                                      !pbp_by_drive_full$stagn.clock & 
                                      pbp_by_drive_full$game_id_half != lead(pbp_by_drive_full$game_id_half))
  ...
  if (length(our.ind.next.end.of.half) > 0){
    pbp_by_drive_full[our.ind.next.end.of.half, "drive_duration_seconds"] <- 60*(pbp_by_drive_full[our.ind.next.end.of.half, c("drive_time_minutes_start")]) +
      pbp_by_drive_full[our.ind.next.end.of.half, c("drive_time_seconds_start") ] 
  }
  
  ## If the CLOCK IS STAGNANT (& duration is 0), & NON 0m:0s END TIME:
  ##    then SET THE DURATION TO NA:
  
  our.ind.stagn.clock <- which(pbp_by_drive_full$drive_duration_seconds == 0 & pbp_by_drive_full$n.plays.with.penalties > 2 & 
                                 !(pbp_by_drive_full$drive_time_minutes_end == 0 & pbp_by_drive_full$drive_time_seconds_end == 0) &
                                 pbp_by_drive_full$stagn.clock)
  
  if (length(our.ind.stagn.clock) > 0){
    pbp_by_drive_full[our.ind.stagn.clock, "drive_duration_seconds"] <- NA
  }
  ...
  print(sort(pbp_by_drive_full$drive_duration_seconds, decreasing = T)[1:10])
  
  print("Number of FULL-QUARTER DRIVES (or higher) with MORE THAN 15 PLAYS:")
  our.ind <- which(pbp_by_drive_full$drive_duration_seconds >= 900 & pbp_by_drive_full$n.plays.no.penalties > 15)
  our.ind
  print(length(our.ind))
  ...
  print(summary(pbp_by_drive_full$tot.yds_after_int))
  
  print("YARDS AFTER INTERCEPTION: ONLY DRIVES with INT!!")
  print(summary(pbp_by_drive_full[pbp_by_drive_full$drive_result_detailed %in% c("Interception Return", "Interception Return Touchdown"), ]$tot.yds_after_int))
  
  
  cat("\n")
  cat("\n")
  print("BY-DRIVE DATA SET SIZE, FINAL")
  print(dim(pbp_by_drive_full))
  cat("\n")
  cat("\n")
  ...
  ########
  ## CHECKING THE EXP POINTS STUFF for DRIVE-BY-DRIVE DATA
  ########
  
  ## Fixing the "ep_after" & EPA for the SCORING drives (from the "pts_by_text")
  ##  Because some could get messed up due to PENALTY plays, which are TRULY OUT OF CONTROL AND TOUGH TO INCORPORATE..
  
  score.ind <- which(pbp_by_drive_full$score_pts_by_text != 0)
  pbp_by_drive_full$ep_after[score.ind] <- pbp_by_drive_full$score_pts_by_text[score.ind]
  pbp_by_drive_full$EPA.diff[score.ind] <- pbp_by_drive_full$ep_after[score.ind] - pbp_by_drive_full$ep_before[score.ind]
  
  
  
  ###
  ## Checking the EP_AFTER
  ##      HANDFUL OF EP_AFTERS (ergo EPA's) are a MESS... NEED TO BE MINDFUL OF THAT..
  ##      !!! EP_BEFORE's are OK !!!
  ###
  ...
  # 
  # i <- 23
  # print(sort(unique(pbp_by_drive_full$drive_result_detailed))[i])
  # our.ind <- which(pbp_by_drive_full$drive_result_detailed == sort(unique(pbp_by_drive_full$drive_result_detailed))[i])
  # print(summary(pbp_by_drive_full$ep_after[our.ind]))
  # print(summary(pbp_by_drive_full$ep_before[our.ind+1]))
  # print(summary(pbp_by_drive_full$EPA.diff[our.ind]))
  # 
  # #  our.ind[which.max(pbp_by_drive_full$ep_after[our.ind])]
  # 
  # View(pbp_by_drive_full[
  #   #c(our.ind[which.min(pbp_by_drive_full$ep_after[our.ind])], our.ind[which.min(pbp_by_drive_full$ep_after[our.ind])]+1)
  #   c(our.ind[which.min(pbp_by_drive_full$ep_before[our.ind+1])], our.ind[which.min(pbp_by_drive_full$ep_before[our.ind+1])]+1)
  #   , ] %>%
  #     select(drive_result_detailed, drive_time_period_start, drive_time_minutes_start, drive_time_seconds_start,
  #            stopped_position, ep_before, ep_after, EPA.sum, EPA.diff,
  #            last.play.text.no.pen, last.play.text.with.pen, first.play.text.no.pen, first.play.text.with.pen))
  # 
  # 
  # View(pbp[pbp$game_id_drive %in% c("400547648-8", "400547648-9"),
  #          c("game_id_drive", "clock.minutes", "clock.seconds", "period", "down", "distance", "yards_to_goal", "play_text", "play_type", "ep_before", "ep_after", "EPA", "pos_team", "def_pos_team")])
  # 
  ...
  save(pbp_by_drive, file=paste0("pbp_UPDATED_w_EPA_Off_Def_ST_", year,".Robj"))
  save(pbp_by_drive_full, file=paste0("pbp_by_drive_w_EPA_Off_Def_ST_", year,".Robj"))
